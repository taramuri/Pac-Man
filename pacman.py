...
def draw_misc():
    # Draws score, power-up indicator, remaining lives, game over and victory messages on the screen
    ...
    if game_over or game_won:
        # Draws game over or victory message with a prompt to restart
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300], 0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
        gameover_text = font.render('Game over! Space bar to restart!' if game_over else 'Victory! Space bar to restart!', True, 'red' if game_over else 'green')
        screen.blit(gameover_text, (100, 300))


def check_collisions(scor, power, power_count, eaten_ghosts):
    # Checks collisions with objects on the game board and updates game state accordingly
    num1 = (HEIGHT - 50) // 32
    num2 = WIDTH // 30
    if 0 < player_x < 870:
...
def draw_board():
    # Draws the game board elements based on their values in the level array
    num1 = ((HEIGHT - 50) // 32)
    num2 = (WIDTH // 30)
...
while run:
    ...
    blinky = Ghost(blinky_x, blinky_y, targets[0], ghost_speeds[0], blinky_img, blinky_direction, blinky_dead, blinky_box, 0, HEIGHT, WIDTH, level, screen)
    inky = Ghost(inky_x, inky_y, targets[1], ghost_speeds[1], inky_img, inky_direction, inky_dead, inky_box, 1, HEIGHT, WIDTH, level, screen)
    pinky = Ghost(pinky_x, pinky_y, targets[2], ghost_speeds[2], pinky_img, pinky_direction, pinky_dead, pinky_box, 2, HEIGHT, WIDTH, level, screen)
    clyde = Ghost(clyde_x, clyde_y, targets[3], ghost_speeds[3], clyde_img, clyde_direction, clyde_dead, clyde_box, 3, HEIGHT, WIDTH, level, screen)

    # Створення атрибутів rect для об'єктів blinky, inky, pinky та clyde
    blinky.rect = blinky_img.get_rect()
    blinky.rect.topleft = (blinky_x, blinky_y)

    inky.rect = inky_img.get_rect()
    inky.rect.topleft = (inky_x, inky_y)

    pinky.rect = pinky_img.get_rect()
    pinky.rect.topleft = (pinky_x, pinky_y)

    clyde.rect = clyde_img.get_rect()
    clyde.rect.topleft = (clyde_x, clyde_y)
